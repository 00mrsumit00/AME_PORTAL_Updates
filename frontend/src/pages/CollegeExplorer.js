...
export default function CollegeExplorer() {
  ...
  const fetchColleges = useCallback(async (proc, search, ct, statList, hs, st, fr) => {
    ...
    try {
      ...
      const res = await api.get('/colleges/process-search', { params });
      const raw = res.data || [];
      setAllColleges(raw);
  ...
  useEffect(() => {
    api.get('/colleges/processes').then(res => setProcesses(res.data.processes)).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get('/colleges/process-metadata', { params: { process: selectedProcess } })
    ...
    <>
      ...
      <div className="ce2-root">
        ...
        <header className="ce2-header">
          <div className="ce2-header-brand">
            <img src="/landing-assets/images/branding/logo1.png" alt="AME Logo" />
            <span className="ce2-header-title">College Explorer</span>
          </div>
        </header>

        {/* Hero */}
        <div className="ce2-hero">
          <div className="ce2-hero-badge"><i className="fas fa-university"></i> {selectedProcess ? `${selectedProcess === 'AIIMS' ? 'AIIMS / JIPMER' : selectedProcess} Admissions Quota` : 'Select Admissions Quota'}</div>
          <h1>College Explorer</h1>
          <p>Browse {selectedProcess ? `${selectedProcess === 'AIIMS' ? 'AIIMS / JIPMER' : selectedProcess} colleges` : 'colleges'} — filter by course, status, hostel availability, fees &amp; more.</p>
        </div>
